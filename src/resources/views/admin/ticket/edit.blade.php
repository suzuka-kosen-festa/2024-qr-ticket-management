                ...
                <div class="bg-white p-4 rounded-md my-4 w-full">
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input type="date" id="event_date" name="event_date"
                            value="{{ old('event_date', $ticket->event_date) }}" required />
                    </div>

                    <!-- 販売開始時間 -->
                    <div class="mb-4">
                        <label for="sale_start_time" class="block text-gray-700">販売開始時間</label>
                        <x-text-input type="datetime-local" id="sale_start_time" name="sale_start_time"
                            value="{{ old('sale_start_time', $ticket->sale_start_time) }}" required />
                    </div>

                    <!-- 終了時間 -->
                    <div class="mb-4">
                        <label for="end_time" class="block text-gray-700">販売終了時間</label>
                        <x-text-input type="datetime-local" id="end_time" name="end_time"
                            value="{{ old('end_time', $ticket->end_time) }}" required />
                    </div>

                    <!-- 残り -->
                    <div class="mb-4">
                        <label for="balance" class="block text-gray-700">残り(注意！！)</label>
                        <x-text-input type="number" id="balance" name="balance"
                            value="{{ old('balance', $ticket->balance) }}" required />
                    </div>
                    ...
                    <div class="grid gap-4">
                        @foreach ($ticketLogList as $log)
                            <div class="p-4 border border-gray-200">
                                <p class="text-gray-700 text-sm font-semibold">日付: <span
                                        class="text-gray-900">{{ $log->created_at->format('Y年m月d日 H:i') }}</span></p>
                                ...
                                <p class="mt-2 text-gray-700 text-sm font-semibold">整理券コード:
                                    <span
                                        class="text-gray-900 font-mono bg-gray-100 px-2 py-1 rounded">{{ $log->unique_code }}</span>
                                </p>
                            </div>
                        @endforeach
                    </div>
